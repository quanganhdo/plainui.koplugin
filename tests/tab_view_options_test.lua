...
test("defaults preserve current behavior", function()
    withSettings(nil, function()
        ...
        assertEqual(options.books.exclude_folders, false)
...
test("set persists valid options", function()
    withSettings(nil, function(getSaved)
        ...
        assertEqual(saved.books.exclude_folders, false)
    end)
end)

test("books can exclude folders", function()
    withSettings(nil, function(getSaved)
        local ok = TabViewOptions.set("books", "exclude_folders", true)
        local saved = getSaved()

        assertEqual(ok, true)
        assertEqual(saved.books.exclude_folders, true)
        assertEqual(TabViewOptions.get("books").exclude_folders, true)
    end)
end)
